...
def create_sales_invoice_from_delivery_note(doc, method):
    """Create a Sales Invoice when a Delivery Note is submitted."""

    # Create a new Sales Invoice
    sales_invoice = frappe.new_doc("Sales Invoice")
    sales_invoice.customer = doc.customer
    sales_invoice.company = doc.company
    sales_invoice.due_date = frappe.utils.add_days(frappe.utils.today(), 30)  # Set due date 30 days from now
    ...
    # Transfer items from Delivery Note to Sales Invoice
    for item in doc.items:
        # Fetch the original Sales Order if available
        sales_order_doc = None
        if item.against_sales_order:
            sales_order_doc = frappe.get_doc("Sales Order", item.against_sales_order)

        # Find the correct Sales Order Item ID (so_detail) if available
        sales_order_item_id = None
        if sales_order_doc:
            for soi in sales_order_doc.items:
                if soi.item_code == item.item_code:
                    sales_order_item_id = soi.name  # Sales Order Item ID
                    break  # Stop once found

        # Append item to Sales Invoice
        sales_invoice_item = sales_invoice.append("items", {
...
@frappe.whitelist()
def on_submit_of_delivery_note(doc, method):
    if doc.custom_sales_order:
        sales_order = frappe.get_doc("Sales Order", doc.custom_sales_order)
        sales_order.custom_delivery_note = doc.name  # Set custom_delivery_note field in Sales Order
