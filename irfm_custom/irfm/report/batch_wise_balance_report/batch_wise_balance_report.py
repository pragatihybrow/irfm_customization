...
# 		)
# 		.groupby(sle.voucher_no, sle.batch_no, sle.item_code, sle.warehouse, sle.pack_size)
...
# 	for field in ["item_code", "batch_no", "company", "pack_size"]:
...
# 	td = get_datetime(filters.to_date + " 23:59:59")
...
# 			& (sle.is_cancelled == 0)
# 			& (sle.has_batch_no == 1)
# 			& (sle.posting_datetime <= td)
# 		)
# 		.groupby(sle.voucher_no, batch_package.batch_no, batch_package.warehouse, sle.pack_size)
# 		.orderby(sle.item_code, sle.warehouse)
# 	)
...
# 			d.batch_no, {}
# 		).setdefault(
# 			d.pack_size or "", frappe._dict({"opening_qty": 0.0, "in_qty": 0.0, "out_qty": 0.0, "bal_qty": 0.0})
# 		)
# 		qty_dict = iwb_map[d.item_code][d.warehouse][d.batch_no][d.pack_size or ""]
	...
	for item in sorted(iwb_map):
		for wh in sorted(iwb_map[item]):
			for batch in sorted(iwb_map[item][wh]):
				for pack_size in sorted(iwb_map[item][wh][batch]):
					qty_dict = iwb_map[item][wh][batch][pack_size]
					if qty_dict.bal_qty != 0:  # Only show rows that have updated balance
						data.append([
							item,
							item_map[item]["item_name"],
							item_map[item]["description"],
...
def get_stock_ledger_entries(filters):
	...
	query = (
		frappe.qb.from_(sle)
		.left_join(delivery_note_item)
		.on(
			sle.voucher_type == "Delivery Note"
			and sle.voucher_detail_no == delivery_note_item.name
		)
		.select(
			sle.item_code,
			sle.warehouse,
			sle.batch_no,
			sle.posting_date,
			fn.Coalesce(delivery_note_item.custom_uoms, sle.pack_size).as_("pack_size"),
			sle.actual_qty
		)
		.where(
			(sle.docstatus < 2)
...
def get_item_warehouse_batch_map(filters, float_precision):
	...
	for d in sle:
		item_dict = iwb_map.setdefault(d.item_code, {}).setdefault(d.warehouse, {}).setdefault(
			d.batch_no, {}
		)
		pack_key = d.pack_size or ""
		qty_dict = item_dict.setdefault(pack_key, frappe._dict({"bal_qty": 0.0}))
